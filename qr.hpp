...
// =============================================================================
// QR class declaration
// =============================================================================

template<int V>
struct Qr {
    constexpr auto side_size() const { return SIDE; }
    constexpr size_t data_size() const { return N_BYTES; }
    const uint8_t* data() const { return code; }
...
// =============================================================================
// Runtime-sized QR code — filled by encode_auto.
// =============================================================================

struct DynQr {
    static constexpr int MAX_SIDE  = 17 + 40 * 4;
    static constexpr int MAX_BYTES = (MAX_SIDE * MAX_SIDE + 7) / 8;

    bool module(int x, int y) const { return get_arr_bit(code, y * side + x); }
...
// Encode str into the smallest QR version (1–40) that fits the data.
// Returns false if the string is too long even for version 40.
bool encode_auto(DynQr& out, const char *str, size_t len, Ecc ecc, int mask = -1);

// =============================================================================
// QR_IMPLEMENTATION — define before including in exactly one translation unit.
// =============================================================================
...
template<int V>
bool try_encode(DynQr& out, const char *str, size_t len, Ecc ecc, int mask)
{
    Qr<V> qr;
    if (qr.encode(str, len, ecc, mask)) {
        out.side   = qr.side_size();
        out.status = true;
        memcpy(out.code, qr.data(), qr.data_size());
        return true;
    }
    if constexpr (V < 40)
        return try_encode<V + 1>(out, str, len, ecc, mask);
    return false;
}
