...
- **Drop shadow**: `shadow{}` struct (`enabled`, `offsetX`, `offsetY`, `blur`, `color[4]` RGBA). Rendered as a 3× separable running-sum **box-blur (≈Gaussian)** of the element shape: the rounded-rect is rasterized into a padded `CAIRO_FORMAT_A8` surface, blurred with `BoxBlurH`/`BoxBlurV` (O(pixels), radius-independent; reciprocal-multiply normalization, clamp-to-edge), then composited as the coloured shadow via `cairo_mask_surface()`. Drawn **before** any wipe/corner/mask clip so it is never clipped; during wipe animations it tracks the clip box rather than the full element bounds. `shadow.blur` maps directly to the Gaussian `sigma` (box radii via `GaussBoxRadii`); the A8 is padded by `sum(box radii) + 1` on every side so the blur never clips. Shadow corner radius uses the element's minimum `cornerRadius` (sharp corners when 0). Rendered **per-frame** — see `OPTIMIZATION_PLAN.md` for the saved caching/downsample follow-ups.
