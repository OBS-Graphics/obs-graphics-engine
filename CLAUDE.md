...
- `UpdateData()` asks `DataPool::DataIfChanged` and applies the records with the animated `SetContent` **only** when that source's cache version moved. `SetDataSource(id)` is how a host re-points a Title at a different source: it assigns `dataSourceId` **and** clears that version counter, which a bare assignment would leave carrying over from the previous source — a freshly registered source is primed to version 1 and a source that never changes stays at version 1, so the compare could read "unchanged" and a Visible Title would keep rendering the old source's record. `Title::Tick` calls it every frame while `Visible` — an unchanged cache costs one integer compare, which is why there is no per-title poll timer anymore.
